...
/*   Updated: 2022/03/24 19:59:20 by alsanche         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	w_key(t_mlx_need *mlx_st)
{
	...
	if (mlx_st->map->reading[p.y - 1][p.x] != '1')
	{
		...
		{
			if (mlx_st->map->reading[p.y - 1][p.x] == 'E')
				if (mlx_st->map->points == mlx_st->map->all_points)
					ft_end_game(mlx_st);
			print_move(p.x, (p.y - 1), 1, mlx_st);
		}
		mlx_st->map->p_place.y = p.y - 1;
		return (1);
	}
	return (0);
}
...
int	a_key(t_mlx_need *mlx_st)
{
	...
	if (mlx_st->map->reading[p.y][p.x - 1] != '1')
	{
		...
		{
			if (mlx_st->map->reading[p.y][p.x - 1] == 'E')
				if (mlx_st->map->points == mlx_st->map->all_points)
					ft_end_game(mlx_st);
			print_move((p.x - 1), p.y, 0, mlx_st);
		}
		mlx_st->map->p_place.x = p.x - 1;
		return (1);
	}
	return (0);
}
